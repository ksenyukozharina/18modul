...
def callback(call):
    if call.data != 'else':
      ...
      bot.send_message(call.message.chat.id, f'Получается: {round(res, 2)}. Впишите  сумму')
      bot.register_next_step_handler(call.message, summa)
    else:
        bot.send_message(call.message.chat.id, 'Введите несколько значений через слэш')
...
def my_currency(message):
    try:
        ...
        res = currency.convert(amount, values[0], values[1])
        bot.send_message(message.chat.id, f'Получается: {round(res, 2)}. Впишите сумму')
        bot.register_next_step_handler(message, summa)
    except Exception:
